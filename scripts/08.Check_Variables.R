...
###########################################################
# CHELSA vs EasyClimate: Pixel Comparison (Jan 1, 2017)
###########################################################
...
# Paths to rasters
# ---------------------------
ec_file <- "E:/TFM_gangas/Climaticas/EasyClimate/DownscaledPrcp2017.tif"
chelsa_file <- "E:/TFM_gangas/Climaticas/CHELSA_pr_01_01_2017_V.2.1.tif"

# ---------------------------
# Iberian Peninsula bounding box
# ---------------------------
...
# ---------------------------
r_ec <- rast(ec_file)
r_ec_day1 <- subset(r_ec, 1)  # first band = Jan 1

r_ch <- rast(chelsa_file)     # CHELSA Jan 1

# ---------------------------
# Crop rasters to Spain
# ---------------------------
r_ec_crop <- crop(r_ec_day1, ext(peninsula_bbox))
r_ch_crop <- crop(r_ch, ext(peninsula_bbox))

# ---------------------------
# Extract values
# ---------------------------
ec_vals <- values(r_ec_crop)
ch_vals <- values(r_ch_crop)

# ---------------------------
# Filter valid values (EasyClimate fill = -32768)
# ---------------------------
valid_idx <- which(ec_vals != -32768 & !is.na(ch_vals))
ec_valid <- ec_vals[valid_idx]
ch_valid <- ch_vals[valid_idx]

# ---------------------------
# Compute absolute and relative differences
# ---------------------------
abs_diff <- ec_valid - ch_valid
rel_diff <- (ec_valid - ch_valid) / ch_valid * 100

# ---------------------------
# Statistics
# ---------------------------
cat("Number of valid pixels:", length(ec_valid), "\n")
cat("ABSOLUTE DIFFERENCES: min =", min(abs_diff),
    " max =", max(abs_diff),
    " mean =", mean(abs_diff),
    " SD =", sd(abs_diff), "\n")
cat("RELATIVE DIFFERENCES (%): min =", min(rel_diff),
    " max =", max(rel_diff),
    " mean =", mean(rel_diff),
    " SD =", sd(rel_diff), "\n")

# ---------------------------
# Optional: histogram of relative differences
# ---------------------------
ggplot(data.frame(rel_diff = rel_diff), aes(x = rel_diff)) +
  geom_histogram(bins = 100, fill = "skyblue", color = "black") +
  theme_minimal() +
  xlab("Relative Difference (%)") +
  ylab("Number of Pixels") +
  ggtitle("Relative Differences CHELSA vs EasyClimate (Jan 1, 2017)")
