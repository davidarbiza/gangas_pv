...
for(sp in species_list){
  ...
  gam_pa_pv_time_paxtime_pvxtime <- bam(
    suitability_beta ~
      s(PA_s) +
      s(PV_s) +
      s(dekad_c, bs = "cc", k = 12) +
  ...
  model_tbl <- data.frame(
    ...
    AIC = c(
      ...
      AIC(gam_pa_pv_time_paxtime),
      AIC(gam_pa_pv_time_pvxtime),
      
      AIC(gam_paxpv_paxtime),
      AIC(gam_paxpv_pvxtime),
      
      AIC(gam_pa_pv_time_paxtime_pvxtime),
    ...
    Deviance_Explained = c(
      ...
      1 - glm_pa_pv$deviance / glm_pa_pv$null.deviance,
      
      summary(gam_pa)$dev.expl,
      summary(gam_pv)$dev.expl,
      summary(gam_pa_pv)$dev.expl,
      
      summary(gam_pa_time)$dev.expl,
      summary(gam_pv_time)$dev.expl,
      summary(gam_pa_pv_time)$dev.expl,
      
      summary(gam_paxpv)$dev.expl,
      summary(gam_paxtime)$dev.expl,
      summary(gam_pvxtime)$dev.expl,
      
      summary(gam_pa_pv_time_paxtime)$dev.expl,
      summary(gam_pa_pv_time_pvxtime)$dev.expl,
      
      summary(gam_paxpv_paxtime)$dev.expl,
      summary(gam_paxpv_pvxtime)$dev.expl,
      
      summary(gam_pa_pv_time_paxtime_pvxtime)$dev.expl,
  ...
  edf_tbl <- data.frame(
    ...
    EDF = c(
      ...
      summary(gam_pa_pv_time)$s.table[1,"edf"],
      summary(gam_pa_pv_time)$s.table[2,"edf"],
      summary(gam_pa_pv_time)$s.table[3,"edf"]
    )
  )
  ...
  smooth_tbl <- as.data.frame(
    summary(gam_pa_pv_time)$s.table
  )
...
for(sp in species_list){
  ...
  pa_new$PA_real <- pa_seq_real
  
  pa_new$fit <- pa_pred$fit
  
  pa_new$lwr <- pa_pred$fit -
    1.96 * pa_pred$se.fit
  
  pa_new$upr <- pa_pred$fit +
    1.96 * pa_pred$se.fit
  ...
  pv_new$PV_real <- pv_seq_real
  
  pv_new$fit <- pv_pred$fit
  
  pv_new$lwr <- pv_pred$fit -
    1.96 * pv_pred$se.fit
  
  pv_new$upr <- pv_pred$fit +
    1.96 * pv_pred$se.fit
  ...
  dekad_new$lwr <- dekad_pred$fit -
    1.96 * dekad_pred$se.fit
  
  dekad_new$upr <- dekad_pred$fit +
    1.96 * dekad_pred$se.fit
...
for(sp in species_list){
  ...
  pa_seq <- seq(min(tbl$PA), max(tbl$PA), length.out = 200)
  ...
  for (d in periods) {
    ...
    newdata <- data.frame(
      PA_s = (pa_seq - PA_mean) / PA_sd,
      PV_s = mean(tbl$PV_s),
      dekad_c = d
    )
    
    pred <- predict(gam_full, newdata = newdata, type = "response")
  ...
  pv_seq <- seq(min(tbl$PV), max(tbl$PV), length.out = 200)
  ...
  p1 <- ggplot(pred_pa, aes(PA, fit, colour = Period, group = Period)) +
    
    geom_line(linewidth = 0.6) +
    
    theme_bw() +
    
    labs(title = if (sp == "PTS") {
      expression(italic("Pterocles alchata") ~ "\nProtected areas × Period")
    } else {
      expression(italic("Pterocles orientalis") ~ "\nProtected areas × Period")
    },
  ...
  #######################################################
  # PHOTOVOLTAIC COVER FIGURE
  #######################################################
  
  p2 <- ggplot(pred_pv, aes(PV, fit, colour = Period, group = Period)) +
...
ggsave(
  file.path(out_dir, "GAM_Interactions.png"),
  final_plot,
  width = 14,
  height = 10,
  dpi = 400
...
valid_static <- !is.na(pa_all)

pa_all <- pa_all[valid_static]

pv_all <- pv_all[valid_static]
...
cat(
  "\nTerritory protected:",
  round(territory_PA_perc, 2),
  "%"
)

cat(
  "\nTerritory photovoltaic:",
  round(territory_PV_perc, 4),
  "%\n"
)
...
for(sp in species_list){
  ...
  for(i in seq_along(files)){
    ...
    hotspot_PA_total <-
      sum(pa_bin, na.rm = TRUE) /
      total_hotspots * 100
    
    hotspot_PV_total <-
      sum(pv_bin, na.rm = TRUE) /
      total_hotspots * 100
    ...
    PA_representation_ratio <-
      hotspot_PA_total /
      territory_PA_perc
    
    PV_representation_ratio <-
      hotspot_PV_total /
      territory_PV_perc
