...
for (ndvi_file in ndvi_files) {
  ...
  if (file.exists(out_file)) {
    message(basename(ndvi_file), " already exists. Skipping.")
    next
  }
...
for (yr in years) {
  ...
  if (is.null(all_files)) {
    message("Could not list contents of ", tgz_path)
    next
  }
  ...
  for (tile in tiles_es) {
    ...
    if (length(match_path) == 0) {
      message("File not found: ", tile_pattern, " in ", yr)
      next
    }
    ...
    if (!file.exists(tif_path)) {
      message("File not properly extracted: ", tif_path)
      next
    }
...
cat("Alredy Reprojected:", length(processed_basenames), "\n")
cat("Not reprojected yet:", length(ndvi_to_process), "\n\n")
