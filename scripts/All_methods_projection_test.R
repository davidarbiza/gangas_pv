...
out_dir <- file.path(model_path, "ALL_METHODS_PROJECTIONS_ENSEMBLE_10DAYS")
...
for(band_i in c(27)){
  ...
  for(model_name in names(models)){
    ...
    for(i in 1:5){
      ...
      out_fold <- file.path(
        "E:/temp_terra",
        paste0(sp, "_", met, "_dekad_", band_i, "_fold_", i, ".tif")
      )
...
# PATH
# ------------------------------------------------------------
base_path <- "E:/TFM_gangas/GPS/ExtractedV.4/CV_models/ALL_METHODS_PROJECTIONS_ENSEMBLE_10DAYS"
...
projection_path <- "E:/TFM_gangas/GPS/ExtractedV.4/CV_models/ALL_METHODS_PROJECTIONS_ENSEMBLE_10DAYS"

gam_path <- file.path(base_path, "Calibration_models")

out_dir <- file.path(base_path, "CALIBRATED_PROJECTIONS_ALL_METHODS")
dir.create(out_dir, showWarnings = FALSE)
...
rf_path <- "E:/TFM_gangas/GPS/ExtractedV.4/CV_models/ALL_METHODS_PROJECTIONS_ENSEMBLE_10DAYS"

# Calibrated maps
cal_path <- "E:/TFM_gangas/GPS/ExtractedV.4/CALIBRATED_PROJECTIONS_ALL_METHODS"
...
# PATHS
# ------------------------------------------------------------
base_path <- "E:/TFM_gangas/GPS/ExtractedV.4/CALIBRATED_PROJECTIONS_ALL_METHODS"
...
for(met in methods){
  ...
  files[[met]] <- list(
    ...
    PTS_sd = sort(list.files("E:/TFM_gangas/GPS/ExtractedV.4/CV_models/ALL_METHODS_PROJECTIONS_ENSEMBLE_10DAYS",
                             pattern = paste0("PTS_", met, "_.*SD\\.tif$"),
                             full.names = TRUE)),
    
    BBS_sd = sort(list.files("E:/TFM_gangas/GPS/ExtractedV.4/CV_models/ALL_METHODS_PROJECTIONS_ENSEMBLE_10DAYS",
